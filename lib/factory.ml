module Make (M : Impl.M) = struct
  ...
  let password ?validate message = Password.prompt message ?validate ~impl:(module M)

  let input ?validate ?default message = Input.prompt message ?validate ?default ~impl:(module M)
